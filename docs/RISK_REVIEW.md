...
11. **A Product Lab deployment could disrupt the existing survey application.** Never deploy this repository as the Product Lab site root. Restrict integration to `/lab/**`, inspect rewrites and shared assets, smoke-test the survey before and after, and keep a path-scoped rollback.
